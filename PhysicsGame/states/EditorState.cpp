...
void EditorState::update(float dt)
{
	...
	if (playMap)
	{
		playMap = false;
		active = false;
		stateManager->addState(new GameState(stateManager, platform, ttmng, cmng, mmng, 1, pms));
	}
...
void EditorState::render()
{
	...
	for (int i = 0; i < 11; i++)
	{
		white->pushToScreen(platform->getRenderer(), Vec2(700+(i * 30), 17), Vec2(1, 10 * 15));
	}
	for (int j = 0; j < 11; j++)
	{
		white->pushToScreen(platform->getRenderer(), Vec2(700, 17+(j * 15)), Vec2(10 * 30, 1));
	}
